...
def diagnoseLR():
    ...
    result, timeConsumed = regression.gradient(
    ...
    plt.ylim(ymin,ymax)
    # 绘制训练样本
    plt.scatter(X[:,1].flatten().A[0], y[:,0].flatten().A[0], marker='x', color='r', linewidths='2')
    #绘制拟合曲线
    ...
    for i in range(m):
        ...
        valErrors[0,i] = regression.J(theta, Xval, yval)
...
def diagnosePR():
    ...
    fitX = np.concatenate((np.ones((fitX.shape[0], 1)), fitX), axis=1)
    fitXX = regression.normalize(np.mat(poly.fit_transform(fitX[:,1:])))
    h = fitXX * theta
    plt.title('Polynomial regression learning curve(lambda=%.3f) \n validation Error=%.3f'%(bestLambda, error))
    # ravel()仍是矩阵，要转化为array，用flatten().A[0] or np.asarray
    plt.scatter(np.asarray(X[:,1].ravel()), y[:,0].flatten().A[0], marker='x', color='r', linewidths=3)
    plt.plot(fitX[:, 1], h, color='b')
    plt.show()
