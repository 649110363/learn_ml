...
if __name__ == '__main__':
    ...
    m,n = X.shape
    X = np.concatenate((np.ones((m,1)),X), axis = 1)#拼接x0
    ...
    title = 'bgd: rate=%0.2f, maxLoop=%d,epsilon=%.3f \n time:%s'%(rate,maxLoop,epsilon,timeConsumed)
    ax = fittingFig.add_subplot(111,title = title)
    # print(X[:, 1].flatten().A[0])
    # print(y[:, 0].flatten().A[0])
